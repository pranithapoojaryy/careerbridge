...
class _StudentAnalyticsScreenState extends ConsumerState<StudentAnalyticsScreen>
  ...
  Widget _buildOverviewTab(Map<String, dynamic> data) {
    ...
    // Calculate interview stats - handle null scores (not yet graded)
    final interviewsTaken = interviews.length;
    final gradedInterviews = interviews
    ...
    // Calculate aptitude performance distribution
    final excellent = aptitude
        .where((a) => ((a['percentage'] as num?)?.toDouble() ?? 0) >= 80)
        .length;
    final good = aptitude.where((a) {
      final score = (a['percentage'] as num?)?.toDouble() ?? 0;
      return score >= 60 && score < 80;
    }).length;
    final needsWork = aptitude
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Aptitude Performance Distribution
          if (aptitude.isNotEmpty) ...[
            Text(
          ...
          // Interview Performance (if available and graded)
          if (gradedInterviews.isNotEmpty) ...[
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${gradedInterviews.length} of $interviewsTaken interviews graded',
                            style: GoogleFonts.outfit(
                              fontSize: 12,
            ...
            const SizedBox(height: 24),
          ] else if (interviewsTaken > 0) ...[
            // Show pending grading message
            Card(
              elevation: 2,
              shadowColor: Colors.black.withValues(alpha: 0.05),
              shape: RoundedRectangleBorder(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '$interviewsTaken mock interviews awaiting faculty grading',
                            style: GoogleFonts.outfit(
                              fontSize: 12,
                              color: Colors.grey[600],
                            ),
  ...
  Widget _buildInterviewTab(List<Map<String, dynamic>> interviews) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final score = (interview['total_score'] as num?)?.toDouble();
        final date =
            DateTime.tryParse(interview['started_at'] ?? '') ?? DateTime.now();
