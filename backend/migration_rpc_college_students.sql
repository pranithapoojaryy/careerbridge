-- Migration to add Security Definer RPCs for College Student Management
-- This bypasses RLS for College Admins to ensure reliable data fetching and verification.

BEGIN;

-- 1. verify_student_v2
CREATE OR REPLACE FUNCTION verify_student_v2(student_uuid UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT
    ...
    p.full_name,
    p.email,
    -- Prefer phone from student_profiles (most likely updated), then profiles
