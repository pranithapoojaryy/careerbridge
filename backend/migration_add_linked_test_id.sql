-- =====================================================
-- MIGRATION: ADD LINKED TEST ID TO QUESTIONS
-- Purpose: Replace fragile JSONB tagging with strict foreign key for custom test questions.
-- =====================================================
...
-- 2. Backfill data from tags
-- Parse the UUID from the "test_id:UUID" tag string
UPDATE public.aptitude_questions
SET linked_test_id = (
    SELECT split_part(element, ':', 2)::uuid
    FROM jsonb_array_elements_text(tags) AS element
    WHERE element LIKE 'test_id:%'
    LIMIT 1
)
WHERE tags::text LIKE '%test_id:%' AND linked_test_id IS NULL;

-- 3. Update RLS Policy to explicitly allow reading questions linked to a test
-- We want students to see questions if they are assigned the test.
-- Current policy: "Authenticated users view active questions" (is_active=true)
-- This is fine, as long as questions are active.
-- But for robustness, we can add a specific policy if needed. 
-- For now, the AdminClient in EdgeFunction bypasses RLS, so just the schema change is sufficient for the logic fix.

-- 4. Update Creator Function
